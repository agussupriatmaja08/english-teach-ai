...
class ChatRepository:
    ...
    def save_message(
        self, user_id: int, role: Literal["User", "System"], message_text: str
    ):
        self.supabase.table("chat_histories").insert(
            {"user_id": user_id, "role": role, "mesage_text": message_text}
        ).execute()
    ...
    def save_user(self, user_id: str, username: str, chat_id: str):
        exist_user = (
    ...
    def last_artifact_by_user_id(self, user_id: int):
        result = (
            self.supabase.table("chat_histories")
            .select("artifact")
            .eq("user_id", user_id)
